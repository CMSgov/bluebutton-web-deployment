...
DOCKER_TAG=${1:-py38buster-an27-tf12-boto3-botocore}

docker build --pull --no-cache --file Dockerfile.cbc-build \
  --build-arg PYTHON_VERSION=${2:-3.8-buster} \
  --build-arg ANSIBLE_VERSION=${3:-2.7} \
